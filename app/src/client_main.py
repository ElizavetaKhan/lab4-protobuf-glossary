...
def run(address="localhost:50051"):
    ...
    print("\n=== 2. Получить одну запись (usability_test) ===")
    try:
        r = stub.GetEntry(pb.GetEntryRequest(key="usability_test"))
    ...
    try:
        ...
        created = stub.PostEntry(
            pb.PostEntryRequest(key="new_method", entry=entry)
        )
    ...
    print("\n=== 5. Удалить запись ===")
    try:
        deleted = stub.DeleteEntry(pb.DeleteEntryRequest(key="new_method"))
